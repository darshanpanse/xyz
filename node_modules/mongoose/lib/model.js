...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    if (!utils.object.hasOwnProperty(obj || {}, '_id')) {
      ...
      // what to update later if more changes are made. the customer
 ...
 *       ]
 *
 *       User.populate(customer, opts, function (err, customer) {
 ...
 *     // });
 *
 *     var customer = { name: 'Indiana Jones', weapon: 389 }
 *     Weapon.populate(customer, { path: 'weapon', model: 'Weapon' }, function (err, customer) {
 *       console.log(customer.weapon.name) // whip
 *     })
 *
 ...
 *         console.log('%s uses a %s', users.name, customer.weapon.name)
 ...
 * query sort order if specified by customer.
 *
 ...
 *   for find, if customer specified sort order, results are assigned directly
...
function assignRawDocsToIdStructure(rawIds, resultDocs, resultOrder, options, localFields, foreignFields, recursed) {
  // honor customer specified sort order
  var newOrder = [];
 ...
 *      b) remove _id from sub docs when customer specified
 *
 *    findOne
 ...
 *
 * background:
 * _ids are left in the query even when customer excludes them so
...
/*!
 * Remove _id from `subdoc` if customer specified "lean" query option
 */
