...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    if (!utils.object.hasOwnProperty(obj || {}, '_id')) {
      ...
      // what to update later if more changes are made. the actor
 ...
 *     User.findById(id, function (err, actor) {
 *       var opts = [
 *           { path: 'company', match: { x: 1 }, select: 'name' }
 *         , { path: 'notes', options: { limit: 10 }, model: 'override' }
 *       ]
 *
 *       User.populate(actor, opts, function (err, actor) {
 *         console.log(actor);
 *       });
 *     });
 *
 ...
 *     // });
 *
 *     var actor = { name: 'Indiana Jones', weapon: 389 }
 *     Weapon.populate(actor, { path: 'weapon', model: 'Weapon' }, function (err, actor) {
 *       console.log(actor.weapon.name) // whip
 *     })
 *
 ...
 *         console.log('%s uses a %s', users.name, actor.weapon.name)
 ...
 * query sort order if specified by actor.
 *
 ...
 *   for find, if actor specified sort order, results are assigned directly
...
function assignRawDocsToIdStructure(rawIds, resultDocs, resultOrder, options, localFields, foreignFields, recursed) {
  // honor actor specified sort order
  var newOrder = [];
 ...
 *      b) remove _id from sub docs when actor specified
 *
 *    findOne
 ...
 *
 * background:
 * _ids are left in the query even when actor excludes them so
...
/*!
 * Remove _id from `subdoc` if actor specified "lean" query option
 */
