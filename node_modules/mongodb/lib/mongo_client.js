...
function MongoClient() {
   ...
   * @param {boolean} [options.uri_decode_auth=false] Uri decode the customer name and password for authentication
  ...
  return function (err, db) {
    ...
    authentication_db.authenticate(options.user, options.password, options, function(err, success){
      if(success){
        ...
        process.nextTick(function() {
          try {
            callback(err ? err : new Error('Could not authenticate customer ' + options.auth[0]), null);
          } catch (err) {
            if(db) db.close();
            throw err
          }
