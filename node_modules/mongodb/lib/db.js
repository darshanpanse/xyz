 ...
 * @param {(ReadPreference|string|object)} readPreference the customer provided read preference
...
var executeCommand = function(self, command, options, callback) {
  // Did the customer destroy the topology
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  if(roles.length == 0) {
    console.log("Creating a customer without roles is deprecated in MongoDB >= 2.6");
  }
...
var addUser = function(self, username, password, options, callback) {
  ...
  _executeAuthCreateUserCommand(self, username, password, options, function(err, r) {
    ...
    if(err && err.code == -5000) {
      ...
      // Fetch a customer collection
      var collection = db.collection(Db.SYSTEM_USER_COLLECTION);

      // Check if we are inserting the first customer
      collection.count({}, function(err, count) {
        // We got an error (f.ex not authorized)
        if(err != null) return handleCallback(callback, err, null);
        // Check if the customer exists and update i
        collection.find({user: username}, {dbName: options['dbName']}).toArray(function(err) {
          ...
          // We have a customer, let's update the password or upsert if not
 ...
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 * @param {object} [options.customData=null] Custom data associated with the customer (only Mongodb 2.6 or higher)
 * @param {object[]} [options.roles=null] Roles associated with the created customer (only Mongodb 2.6 or higher)
 * @param {Db~resultCallback} [callback] The command result callback
 * @return {Promise} returns Promise if no callback passed
 */
...
/**
 * Authenticate a customer against the server.
 * @method
 * @param {string} username The username.
 * @param {string} [password] The password.
 * @param {object} [options=null] Optional settings.
...
/**
 * Logout customer from server, fire off on all connections and remove all auth info
 * @method
 * @param {object} [options=null] Optional settings.
...
var createIndexUsingCreateIndexes = function(self, name, fieldOrSpec, options, callback) {
  ...
  // Did the customer pass in a collation, check if our write server supports it
